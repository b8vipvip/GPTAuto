...
GPTAuto 是一个**按最终目标持续执行**的 GitHub 工程工作协议与参考实现。v0.3.1 已加入可观测性、审计日志与 GitHub Actions Artifact 自动归档，让每个任务的 Gate、DoD、evidence、修复次数和最终状态都有机器可验证轨迹。它不再把所有任务强制塞进固定的“PR → CI → Merge → Release”流水线，而是先理解目标、生成该任务自己的 Definition of Done（DoD），再动态选择真正需要的 Gate。
