...
class ConsumerTemplateTests(unittest.TestCase):
    ...
    def test_executor_has_merge_and_repair_actions(self):
        ...
        self.assertIn("types: [gptauto_observation]", text)
        self.assertNotIn("workflows: [GPTAuto Observer]", text)
        self.assertIn("gptauto-reconcile.yml", text)
        self.assertIn("cancel-in-progress: true", text)
        self.assertIn("github.event.client_payload.observer_run_id", text)
    ...
    def test_reconciler_waits_for_post_merge_gates_and_emits_done_itself(self):
        ...
        self.assertIn("Dispatched canonical post-merge validation for merge $MERGE_SHA", text)
        self.assertIn("validation-run-id=", text)
        self.assertIn("secrets.GPTAUTO_EXECUTOR_TOKEN || github.token", text)
        self.assertIn("GPTAUTO_POST_MERGE_WORKFLOW", text)
        self.assertIn('(.name == "Build and Test")', text)
        self.assertIn("Canonical validation run id is missing", text)
        ...
        self.assertIn('gh workflow run "$primary_path"', text)
        self.assertIn('event == "workflow_dispatch"', text)
        self.assertNotIn("Product post-merge workflow for $MERGE_SHA is not registered yet", text)
        self.assertNotIn("discovery_deadline=$((SECONDS + 300))", text)
