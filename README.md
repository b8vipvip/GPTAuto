...
GPTAuto 是一个**按最终目标持续执行**的 GitHub 工程工作协议与参考实现。v0.6.1 在 v0.6 的仓库侧 Observer 基础上补齐了跨事件任务关联与完成语义：PR、合并后的 main push、main CI、Release 会先反查并绑定到同一个 PR TASK_ID；PR merged 不再直接等于 DONE。版本号或“发布 / 正式版 / release / publish”语义会恢复 Release 型 DoD，只有 post-merge CI 与 Release 都成功后才进入 DONE。
